  ...
  return (
      ...
      <S.StoresList>
        {stores.map((store, index) => (
          <S.StoreButton key={store.id} onClick={() => handleStoreButtonClick(store.id)}>
            <S.StorePic />
            <S.StoreInfoList>
              <S.StoreInfo1>
                <div>{index + 1}위</div>
              ...
              <S.StoreInfo2>
                <div>★ {store.rate} ({store.reviewCnt})</div>
                <div>{store.minDeliveryTime}분~{store.maxDeliveryTime}분 · 배달비 {store.deliveryFee}원</div>
              </S.StoreInfo2>
            </S.StoreInfoList>
          </S.StoreButton>
        ))}
