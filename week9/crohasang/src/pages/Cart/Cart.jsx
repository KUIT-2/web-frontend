...
const Cart = () => {
  ...
  const countMenu = useCartStore((state) => state.countMenu);
  const navigate = useNavigate();

  useEffect(() => {
    menus.forEach((menu) => countMenu(menu.id));
  }, [menus, countMenu]);
  ...
  return (
    <S.WrapCart>
      <S.CartTop>
        ...
        <S.CancelOrder onClick={() => handleCancelOrderClick()}>
          주문취소
        </S.CancelOrder>
      </S.CartTop>
      ...
      <S.OrderList>
        <S.NameandReached>
          ...
          {isBuyButtonActive ? null : (
            <S.WrapNotReached>
                <S.PricenotReached>최소금액 미달</S.PricenotReached>
                <S.notReachedCircle>
                  <S.notReachedExclamation>!</S.notReachedExclamation>
                </S.notReachedCircle>
              </S.WrapNotReached>
          )}
        </S.NameandReached>
        <S.MenuDiv>
          ...
          <S.AddMore onClick={() => GoBackClick(store?.id)}>
            더 담기 +
          </S.AddMore>
        </S.MenuDiv>
      </S.OrderList>
